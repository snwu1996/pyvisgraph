"""Generate 10 benchmark KML obstacle maps of increasing complexity.

Writes examples/kml/benchmark/map_01.kml ... map_10.kml. Each map places
jittered convex n-gons on non-overlapping grid cells inside a small lon/lat
extent near the existing KML examples (6 E / 51 N). Deterministic: a fixed
seed per map makes the files reproducible fixtures for
...
def main():
    ...
    for idx, n_polygons in enumerate(POLYGON_COUNTS, start=1):
        ...
        polygons = make_map(n_polygons, rng)
        placemarks = ''.join(
